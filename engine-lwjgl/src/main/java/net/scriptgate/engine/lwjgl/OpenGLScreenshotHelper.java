...
import net.scriptgate.engine.lwjgl.util.ArrayUtil;
...
public class OpenGLScreenshotHelper {
    ...
    public static BufferedImage getScreenshot(int x, int y, int width, int height) {
        ...
        buffer.get(pixels);
        ArrayUtil.flipVertically(pixels, width, height);
    ...
    public static String saveScreenshot(String fileName, int displayWidth, int displayHeight) {
        try {
            ...
            if (fileName == null) {
                screenshotFile = FileUtil.getUniqueFileNameWithTimestamp(screenshotFolder, "png");
            } else {
